...
import { launchCamera, launchImageLibrary } from 'react-native-image-picker';
    ...
    const Try=async()=>{
     ...
     const sendMessage = async () => {
            ...
            return (
                // random= props.navigation.getParam('UserName')
                <View style={{ flex: 1, backgroundColor: '#000' }}>
                    <AppHeader title={userName} navigation={props.navigation} onPress={() => logOut()} />
                    <FlatList
                        inverted
                    ...
                    <View style={{ bottom: 0, height: 50, width: '100%', position: 'absolute', flexDirection: 'row' }}>
                        <TouchableOpacity style={{ width: '10%', justifyContent: 'center', alignItems: 'center', marginRight: 5 }} onPress={() =>openGallery()}>
                            <Image source={IMAGES.CAMERA} style={{height:30,width:40,borderRadius:40/2,resizeMode:'contain'}} />
                        </TouchableOpacity>
                        <View style={{ width: '75%', justifyContent: 'center' }}>
                            <TextInput value={message} onChangeText={(text) => setMessage(text)} placeholder="Enter Message" placeholderTextColor="#000" style={{ height: 40, borderRadius: 20, backgroundColor: '#ccc' }} />
                        </View>
                        <TouchableOpacity style={{ width: '10%', justifyContent: 'center', alignItems: 'center', marginLeft: 5 }} onPress={() => sendMessage()}>
                           <Image source={IMAGES.SEND} style={{height:30,width:40,borderRadius:40/2,resizeMode:'contain'}}/>
                        </TouchableOpacity>
                    </View>
                </View>
            )
